...
import { ErrorState, LoadingState, PageHeader, ResultValue, TechnicalDetails } from "./ui.jsx";
...
export function DemoPage({ refreshShell, demoReplay }) {
  ...
  // Developer instrumentation, not part of the demo story.
  ...
  // The guided walkthrough opens this real exit dialog; promotion is unchanged.
  useEffect(() => onTourEvent((detail) => {
    if (detail.kind === "request-exit") setExitOpen(true);
  }), []);
  useEffect(() => {
  ...
  const exit = async (promote) => {
    ...
    try {
      ...
      setDemoSessionId(""); setExitOpen(false); await refreshShell?.(); window.location.hash = promote ? "setup/space" : "dashboard"; return result;
    } catch (nextError) { setError(nextError); } finally { setBusy(false); }
  };
  ...
  return <>
    <PageHeader title="Aisle 04" description="Four cameras, one space."
      actions={<button className="button button-secondary" data-demo-tour="demo-exit" onClick={() => setExitOpen(true)}>Exit demo</button>} />
    <div className="demo-status-strip"><span><i className={session.status === "running" ? "active" : ""} /> {session.status}</span><span>{replay.videoTime.toFixed(3)} / {session.duration_s.toFixed(3)} sec</span><button onClick={() => control(session.status === "running" ? "pause" : "start")} disabled={busy}>{session.status === "running" ? <Pause size={13} /> : <Play size={13} />} {session.status === "running" ? "Pause" : "Play"}</button></div>
    ...
    <div className="demo-video-grid" data-demo-tour="demo-camera-grid">{CAMERA_KEYS.map((key, index) => <figure key={key} data-demo-tour={`camera-${index + 1}-tile`}><div className="demo-video-stage"><video ref={(node) => { videos.current[key] = node; }} data-camera-key={key} src={assetUrl(`/demo/media/${key}.mp4`)} muted playsInline preload="auto" onLoadedMetadata={(event) => { event.currentTarget.currentTime = replay.videoTime; }} onTimeUpdate={(event) => { const mediaTime = event.currentTarget.currentTime; setPresented((current) => ({ ...current, [key]: mediaTime })); }} /><VideoEvidenceOverlay definition={session.result?.camera_overlays?.[key]} frame={evidence[key]?.frames?.[frameIndex]} fusedEntities={replay.entities} /></div><figcaption><strong>Camera {index + 1}</strong></figcaption></figure>)}</div>
    <div className="demo-outcome-grid"><section className={`panel demo-occupancy ${known && count >= 2 ? "threshold" : ""}`} data-demo-tour="demo-occupancy"><h2>People in Aisle 04</h2><ResultValue value={count} quality={replay?.kpi?.quality} /></section><section className="panel demo-alert-panel" data-demo-tour="demo-alert"><h2>Alert</h2><strong>{alerts.length ? "Triggered" : "Waiting"}</strong><p>{alerts.length ? "The aisle reached the alert threshold." : "Nothing to report yet."}</p></section></div>
    <section className="panel demo-timeline"><TechnicalDetails summary="What the demo set up"><ol>{session.action_log.map((item) => <li key={item.name}><CheckCircle2 size={16} aria-hidden="true" /><div><strong>{item.name}</strong><p>{item.explanation}</p><code>{JSON.stringify(item.result)}</code></div></li>)}</ol></TechnicalDetails></section>
    <section className="panel demo-explore"><div><h2>See the same moment elsewhere</h2></div><div>{[["live","Live"],["dashboard","Dashboard"],["setup/space","Space"],["observations","Observations"],["sources","Sources"]].map(([route, label]) => <a key={route} className="button button-secondary" href={`#${route}`}>{label}<ExternalLink size={12} /></a>)}</div></section>
    {(error || demoReplay?.error) && <div className="inline-warning">{(error || demoReplay.error).message}</div>}
    {exitOpen && <div className="modal-backdrop"><section className="modal demo-exit"><h2>Leave the demo</h2><p>You can keep the space and cameras the demo set up, or discard everything.</p><ul><li>The aisle, its saved question, dashboard and alert stay in the demo.</li></ul><label className="check-row"><input type="checkbox" checked={includeObservations} onChange={(event) => setIncludeObservations(event.target.checked)} /> Also copy the recorded observations</label><div className="panel-footer"><button className="button button-ghost" onClick={() => setExitOpen(false)}>Cancel</button><button className="button danger" disabled={busy} onClick={() => exit(false)}><Trash2 size={14} /> Discard demo</button><button className="button button-primary" disabled={busy} onClick={() => exit(true)}><Save size={14} aria-hidden="true" /> Keep the setup</button></div></section></div>}
  </>;
}
