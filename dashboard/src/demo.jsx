...
export function DemoPage({ refreshShell, demoReplay }) {
  ...
  return <>
    ...
    <div className="demo-outcome-grid"><section className={`panel demo-occupancy ${known && count >= 2 ? "threshold" : ""}`} data-demo-tour="demo-occupancy"><span className="tiny-label">Cached real saved-query result</span><strong>{count ?? "—"}</strong><h2>Fused people in Aisle 04</h2><p>Quality: {replay?.kpi?.quality || "unknown"} · evidence from {replay?.kpi?.evidence?.source_count || 0} sources · derived at {sample?.video_time_s?.toFixed(3) ?? "—"}s.</p></section><section className="panel" data-demo-tour="demo-alert"><span className="tiny-label">Cached real alert evaluation</span><h2>{alerts.length ? "Threshold event recorded" : "Waiting for ≥ 2 people"}</h2><p>{alerts.at(-1)?.message || "Only alert events whose derived time is at or before the master clock are visible."}</p>{alerts.length > 0 && <small>Current count: {count} · threshold event at {alerts.at(-1).video_time_s.toFixed(3)}s</small>}</section></div>
