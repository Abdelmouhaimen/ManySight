...
def test_managed_credentials_are_encrypted_and_hidden(client, isolated_db, monkeypatch):
    monkeypatch.setenv("STORELENS_CREDENTIAL_KEY", KEY)
    ...
    assert stored["encrypted_payload"].startswith("v1:")
...
def test_privileged_resolution_is_header_only_and_ignores_public_reads(client, monkeypatch):
    ...
    assert client.get(path, headers={"X-StoreLens-Credential-Key": "wrong"}).status_code == 401
    resolved = client.get(path, headers={"X-StoreLens-Credential-Key": "resolve-only"})
...
def test_edit_preserves_replaces_and_clears_credentials(client, monkeypatch):
    monkeypatch.setenv("STORELENS_CREDENTIAL_KEY", KEY)
    monkeypatch.setenv("STORELENS_CREDENTIAL_ACCESS_KEY", "resolve")
    source = client.post("/api/v1/sources", json=managed_rtsp()).json()
    sid = source["id"]
    headers = {"X-StoreLens-Credential-Key": "resolve"}
...
def test_invalid_edit_does_not_destroy_existing_credentials(client, monkeypatch):
    ...
    resolved = client.get(f"/api/v1/sources/{sid}/connection", headers={"X-StoreLens-Credential-Key": "resolve"}).json()
...
def test_wrong_encryption_key_cannot_decrypt_existing_credentials(client, monkeypatch):
    ...
    monkeypatch.setenv("STORELENS_CREDENTIAL_ACCESS_KEY", "resolve")
    source = client.post("/api/v1/sources", json=managed_rtsp()).json()
    monkeypatch.setenv("STORELENS_CREDENTIAL_KEY", base64.urlsafe_b64encode(b"x" * 32).decode())
    response = client.get(
        f"/api/v1/sources/{source['id']}/connection",
        headers={"X-StoreLens-Credential-Key": "resolve"},
    )
...
def test_managed_noncredential_source_kinds_persist(client, kind, connection):
    response = client.post("/api/v1/sources", json={
        "name": kind, "kind": kind, "connection_management": "storelens_managed",
...
@pytest.mark.parametrize("body", [
    {"name": "bad webcam", "kind": "webcam", "connection_management": "storelens_managed", "connection": {"device_index": -1}},
    {"name": "bad rtsp", "kind": "rtsp", "connection_management": "storelens_managed", "connection": {"host": "rtsp://bad", "port": 554, "path": "/live"}},
    {"name": "embedded", "kind": "http", "connection_management": "storelens_managed", "connection": {"url": "http://user:pass@camera/video"}},
    {"name": "token URL", "kind": "http", "connection_management": "storelens_managed", "connection": {"url": "http://camera/video?token=secret"}},
    {"name": "wrong fields", "kind": "file", "connection_management": "storelens_managed", "connection": {"url": "http://camera/video"}},
])
